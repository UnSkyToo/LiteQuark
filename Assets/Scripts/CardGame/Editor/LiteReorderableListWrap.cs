...
namespace LiteEditor
{
    internal sealed class LiteReorderableListWrap
    {
        ...
        public IList Draw(GUIContent title, object[] attrs = null)
        {
            if (List_ == null)
            {
                return null;
        ...
        {
            var title = new GUIContent($"Element {index}");
            
            if (ElementType_ == typeof(bool))
            {
                return EditorGUI.Toggle(rect, title, (bool)element);
            }

            if (ElementType_ == typeof(int))
            {
                return EditorGUI.IntField(rect, title, (int)element);
            }

            if (ElementType_ == typeof(float))
            {
                return EditorGUI.FloatField(rect, title, (float)element);
            }

            if (ElementType_ == typeof(string))
            {
                return EditorGUI.TextField(rect, title, (string)element);
            }

            if (ElementType_.IsEnum)
            {
                return EditorGUI.EnumPopup(rect, title, (Enum)element);
            }
